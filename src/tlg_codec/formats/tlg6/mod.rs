...
use crate::tlg_codec::formats::constants::{TLG6_H_BLOCK_SIZE, TLG6_W_BLOCK_SIZE, TLG_MAGIC_SIZE};
use crate::tlg_codec::golomb::decode_golomb;
...
impl Tlg6 {
    ...
    pub fn from_bytes(buf: &[u8]) -> anyhow::Result<Self> {
        let mut position = TLG_MAGIC_SIZE;
        let offset = &mut position;
    ...
    pub fn to_rgba_image(&self) -> anyhow::Result<RgbaImage> {
        ...
        for (i, line) in self.lines.iter().enumerate() {
            ...
            for (c, bits) in line.bits.iter().enumerate() {
                match bits.method {
                    ...
                    method => bail!("unsupported TLG6 entropy coding method: {method}"),
                }
            }
        ...
        .context("invalid TLG6 output buffer")
    }
